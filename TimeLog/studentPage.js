...
onSubmit = () => {
    ...
    if (assignments.length > 0) {
        for (n = 0; n < assignments.length + 1; n++) {
            assignmentsNamesList.push(document.getElementsByClassName("nameStyle")[n].innerHTML);
            assignmentTimeList.push(document.getElementsByClassName("form-control")[n].value);
...
window.onload = function () {
    ...
    if (assignments.length > 0) {
        for (n = 0; n < assignments.length; n++) {
            ...
            document.getElementById("assignmentName" + String(n)).innerHTML = String(assignments[n]);
    ...
    if (assignmentTimeList.length > 0) {
        for (n = 0; n < assignments.length + 1; n++) {
            document.getElementsByClassName("form-control")[n].value = String(assignmentTimeList[n]);
        }
    }
};
